    ...
    <>
      <div className="min-h-screen flex justify-center pt-36 md:pt-48 p-6">
        <div className="w-full max-w-xl text-center -mt-24 md:-mt-32">
          ...
          <h1 className="brand-glow text-6xl font-bold tracking-tight md:text-8xl">Notz</h1>

          <div className="mt-4 text-base anim-fadeUp anim-delay-1" style={{ color: "var(--muted)" }}>
            x
          </div>
          ...
          <a
            ...
            className="mt-1 inline-block text-2xl transition hover:opacity-80 md:text-3xl anim-fadeUp anim-delay-2"
            style={{
              color: "var(--fg)",
          ...
          <div className="mt-10 grid gap-3 anim-fadeUp anim-delay-3 sm:grid-cols-2">
            <button
              onClick={onLogin}
              disabled={isBusy}
              className="flex items-center justify-center gap-3 rounded-xl border px-6 py-3 transition hover:scale-[1.03] active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-70"
              style={{
                background: "var(--card)",
          ...
          <div className="mt-4 grid gap-3 anim-fadeUp anim-delay-3 sm:grid-cols-2">
            <button
              onClick={() => openModal("signin")}
              disabled={isBusy}
              className="button-primary rounded-xl border px-6 py-3 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-70"
            >
              Sign in
            </button>
            <button
              ...
              disabled={isBusy}
              className="button-neutral rounded-xl border px-6 py-3 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-70"
            >
              Sign up
            </button>
          </div>
          ...
          {authError ? (
            <p className="mt-4 text-sm anim-fadeUp anim-delay-3" style={{ color: "#d66a6a" }}>
              {authError}
            </p>
          ) : null}
        </div>
      ...
      >
        <div
          ...
          style={{
            opacity: isModalOpen ? 1 : 0,
            transform: isModalOpen ? "translateY(0) scale(1)" : "translateY(12px) scale(0.98)",
            transitionDuration: "180ms",
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm" style={{ color: "var(--muted)" }}>
                {mode === "signin" ? "Use your email and password to access Notz." : "Create a password-based Notz account."}
              </p>
            </div>
            <button
              type="button"
