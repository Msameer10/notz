...
export default function NoticeBoardDrawer({
  ...
  return (
    <>
      ...
      >
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-[color:var(--border)] px-4 py-4 sm:px-5">
          <div className="min-w-0">
            <h2 className="truncate text-lg font-semibold sm:text-xl">Notice Board</h2>
          </div>

          <div className="flex items-center justify-end gap-2 whitespace-nowrap">
            <button onClick={add} className="button-primary rounded-lg border px-2.5 py-2 text-sm font-medium transition-colors sm:px-3">
              + Item
            </button>
            <button onClick={onClose} className="button-neutral rounded-lg border px-2.5 py-2 text-sm font-medium transition-colors sm:px-3">
              Close
            </button>
          </div>
        </div>
